...
# Mock SAP Business One Service Layer for the financials connector tests. Session
...
listener http:Listener mockListener = new (9097,
    secureSocket = {
        key: {
...
service / on mockListener {
    ...
    isolated resource function 'default [string... paths](http:Request req) returns http:Response|error {
        ...
        if method == "GET" && path == "/b1s/v1/AccountCategory" {
