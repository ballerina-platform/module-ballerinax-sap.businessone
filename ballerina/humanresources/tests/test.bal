...
// under the License.
import ballerinax/sap.businessone.humanresources.mock as _;
...
final string serviceUrl = isLiveServer ? os:getEnv("B1_SERVICE_URL") : "https://localhost:9099/b1s/v1";
...
function initializeClient() returns error? {
    if isLiveServer {
        log:printInfo("Running humanresources connector tests against a live Service Layer");
        b1 = check new ({companyDb, username, password}, {secureSocket: {enable: false}}, serviceUrl);
    } else {
        log:printInfo("Running humanresources connector tests against the mock Service Layer");
...
function testList() returns error? {
    EmployeeIDTypeCollectionResponse response = check b1->listEmployeeIDType();
